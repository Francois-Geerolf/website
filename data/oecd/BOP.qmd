...
```{r GRC-FRA-USA-EU27-2008-smoothed}
BOP %>%
  filter(MEASURE == "CA",
         REF_AREA %in% c("FRA", "GRC", "GBR", "ITA", "ESP"),
         FREQ == "Q",
         UNIT_MEASURE == "PT_B1GQ") %>%
  quarter_to_date() %>%
  left_join(REF_AREA, by = "REF_AREA") %>%
  ...
  scale_x_date(breaks = c(seq(1990, 2100, 1)) %>% paste0("-01-01") %>% as.Date,
               labels = date_format("%Y")) +
  scale_y_continuous(breaks = 0.01*seq(-60, 60, 1),
                     labels = scales::percent_format(accuracy = 1)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black")
...
BOP %>%
  filter(MEASURE == "CA",
         REF_AREA %in% c("FRA", "GRC", "GBR", "EU27_2020", "ITA", "ESP"),
         FREQ == "Q",
         UNIT_MEASURE == "PT_B1GQ") %>%
  quarter_to_date() %>%
  left_join(REF_AREA, by = "REF_AREA") %>%
